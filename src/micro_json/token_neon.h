...
__FORCE_INLINE__ void Neon_tokenize_1(MJSParsedData *parsed_data) {
 ...
 while((parsed_data->current+16) < parsed_data->end) {
  ...
  int8x16_t any_equals = vorrq_s8(tmp1, tmp2);
...
__FORCE_INLINE__ void Neon_read_json_object_value(MJSParsedData *parsed_data) {
 ...
 while((parsed_data->current+16) < parsed_data->end) {
  current = vld1q_s8((const signed char*)parsed_data->current);
  int8x16_t any_equals = vorrq_s8(vorrq_s8(vorrq_s8(vorrq_s8(vceqq_s8(current, new_line_16), vceqq_s8(current, t_16)), vceqq_s8(current, f_16)), vceqq_s8(current, n_16)), vceqq_s8(current, double_quote_16));
